...
class UploadModal {
  ...
  constructor(el) {
    ...
    this.el?.addEventListener('#file')?.addEventListener('change', this.fileHandle.bind(this))
  }
  ...
  async copy() {
    ...
    if (!this.isCopying && copyButton) {
      ...
      await new Promis(res => setTimeout(res, 1000))
      // reenable
  ...
  fileHandle(e) {
    return new Promis(() => {
      ...
      if (target?.files.length) {
        ...
        reader.readAsDataURL(target.file[0])
      }
    })
  }
  ...
  fileReset() {
    ...
    if (fileField) fileFiled.value = null
